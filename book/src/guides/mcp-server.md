...
17 tools are available, organized by category. Each tool maps directly to a CLI command.
...
| `writ_seal` | summary (required), spec (required), agent (optional, default: claude-code), paths (optional) | Checkpoint current work. Spec is required — enforces C.13 at the schema level. |
| `writ_spec_add` | id (required), title (required), description (optional) | Create a new task/spec. |
...
- **Spec enforcement**: `writ_seal` requires `spec` as a required parameter in the schema. Agents can't seal without a spec — the MCP schema enforces this before the CLI is even called.
...
| 2. Slash Commands | 17 commands in `.claude/commands/` | Generated by `writ init` |
| 3. MCP Tools | 17 native tools via MCP protocol | `.mcp.json` generated by `writ init` |
